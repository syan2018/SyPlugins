...
#### 3.1 ✅ 新增：统一状态入口（StateComponent）
为避免后续系统（SyCombat/任务/AI/交互）直接依赖“某个具体后端”（Generic State / GAS / 自研数值），SyCore 将 **`USyStateComponent` 作为唯一入口**：

- **`USyStateComponent`**
  - **唯一入口**：上层系统只通过 StateComponent 进行 Query/Apply
  - **可插拔后端**：通过 `USyStateBackendBase` 子对象扩展（例如 GAS 后端作为 SyCombat 插件内的 `SyGASBridge` 模块提供）
  - **标准化配置**：推荐使用 `USyStateProfile`（统一 DefaultInitData + BackendTypes/BackendInstances）
  - **无隐式回退**：必须显式添加后端对象或 Profile

推荐搭配：
- **通用旧状态后端**：`USyGenericStateBackend`
- **GAS 后端**：`USyGASStateBackend`（SyCombat 内 `SyGASBridge` 模块）

推荐流程：
1. 创建 `USyStateProfile` 资产（DefaultInitData + BackendTypes/BackendInstances）
2. 在 `USyStateComponent.StateProfile` 指定该资产
