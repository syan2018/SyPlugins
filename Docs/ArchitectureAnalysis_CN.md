...
└─────────────────────────────────────────────────────┘
                           ▲
┌─────────────────────────────────────────────────────┐
│          核心层 (Core Layer) - SyCore               │
│ ┌──────────────┐  ┌──────────────┐  ┌──────────────┐│
│ │    Entity    │  │    State     │  │  Messaging   ││
│ └──────────────┘  └──────────────┘  └──────────────┘│
│                 Foundation                          │
└─────────────────────────────────────────────────────┘
```
...
#### 问题描述
状态管理相关功能曾被分割为三个独立模块 (SyStateCore, SyOperation, SyStateManager)，导致理解成本高和维护负担。

#### 解决状态
**已合并**。
SyEntity, SyStateSystem, SyOperation 等模块已全部合并入 `SyCore` 模块中。
现在的 `SyCore` 包含了 Entity, State, Messaging, Foundation 四大子系统，结构更加紧凑，编译依赖更简单。
